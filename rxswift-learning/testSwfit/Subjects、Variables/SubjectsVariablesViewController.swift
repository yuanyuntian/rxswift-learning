...
class SubjectsVariablesViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let subject2 = BehaviorSubject(value: "11111")
    ...
    //ReplaySubject 创建时候需要一个bufferSize，指定他的event缓存个数。缓存最后发出的几个event。如果一个订阅者订阅了这个subject，这个订阅者会立即收到之前的几个event，然后接收到后面正常发出的event。如果订阅者订阅了已经终结的subject，除了收到缓存的.next event之外，还会收到那个终结的.error或.completed event。
        ...
        let variable = BehaviorRelay<String>.init(value: "")
        variable.accept("222111")
        variable.asObservable().subscribe {
            print("第一次订阅：",$0)
        }.disposed(by: disposeBag)
        variable.accept("333111")
        variable.asObservable().subscribe {
            print("第二次订阅：",$0)
        }.disposed(by: disposeBag)
        variable.accept("444111")
